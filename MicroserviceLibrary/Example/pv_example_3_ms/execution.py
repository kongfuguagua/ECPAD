...
#         test_x = torch.autograd.Variable(input_x).to(device).unsqueeze(0)
#         test_y = torch.autograd.Variable(label_y)
#         test_x=test_x.permute(0,3,1,2)   #调整输入维度
#         model = torch.load('./pv_infer.pth').to(device)
#         out = model(test_x)
#         print('Infer value:', out , 'True value:', test_y)
#         infer_reference = [out, test_y]
...
class batch_net(nn.Module):
    ...
    def NNLoad(self):
        self.model = torch.load('./pv_infer.pth').to(self.DEVICE)
        self.model.eval()
        # self.DEVICE = 'cpcu'
